...
//#3
   ...
   create(req, res, next){
    ...
    advertisementQueries.addTopic(newAdvertisement, (err, advertisement) => {
      if(err){
  ...
  edit(req, res, next){
    advertisementQueries.getTopic(req.params.id, (err, advertisement) => {
      if(err || advertisement == null){
        res.redirect(404, "/");
      } else {
 ...
 edit(req, res, next){
  advertisementQueries.getAdvertisement(req.params.id, (err, advertisement) => {
    if(err || advertisement == null){
      ...
      res.render("topics/edit", {advertisement});
    }
  });
 }
}
